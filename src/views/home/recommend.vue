<template>
  <page-content style="padding-top: 0">
    <template v-show="recommend">
      <div class="recommend">
        ...
        <list>
          <list-item v-for="(tl, idx) in recommend" :key="tl.id">
            ...
            <router-link :to="{name: 'musicPlayer', query: {id: tl.id}}" tag="div" class="list-item-holder dense">
              <list-item-inner>
                <div class="primary-title f-elpsl-1">{{tl.name}}</div>
                <div class="third-title f-elpsl-1">{{tl.artists}} - {{tl.album}}</div>
              </list-item-inner>
    ...
    <loading-page v-show="!recommend"></loading-page>
  </page-content>
</template>
